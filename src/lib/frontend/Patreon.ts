...
export async function initializePatreon() {
    const content = await readFile('patrons.json');
    const patrons = JSON.parse(content.toString('utf8')) as PatronList;

    const $container = $('#patrons');
    const becomePatron = $container.find('button');
    becomePatron.click(() => {
        window.require('nw.gui').Shell.openExternal(PATREON_LINK);
    });

    const activeList = $container.find('#activeList');
    const formerList = $container.find('#formerList');
