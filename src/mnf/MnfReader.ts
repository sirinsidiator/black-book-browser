...
const MNF_FIELD_DEFINITIONS: FieldDefinition[] = [
    { type: FieldType.UINT16, bigEndian: true, name: 'numArchiveFiles' },
    { type: FieldType.UINT16_ARRAY, bigEndian: true, name: 'archiveFileLookup' },
    { type: FieldType.UINT32, bigEndian: true },
    { type: FieldType.UINT8 },
    { type: FieldType.UINT32, name: 'eofOffset' }, // this is the only little endian between all the big endians
...
async function extractContent(archive: MnfArchive) {
    let named = archive.data.named;
    let fileCount = named['fileCount2'].value;
    ...
    let block0 = new BufferReader(named['block0'].value);
    let block1 = new BufferReader(named['block1'].value);
    let block2 = new BufferReader(named['block2'].value);
    ...
    for (let i = 0; i < fileCount; ++i) {
        let entry = new MnfEntry();

        let indexField = new Field(ENTRY_FIELD_DEFINITION_INDEX, -1);
        indexField.value = i;
        entry.data.add(indexField);

        let rowCountField = new Field(BLOCK0_FIELD_DEFINITION_ROW_COUNT, -1);
        rowCountField.value = 0;
        entry.data.add(rowCountField);

        let idField = new Field(BLOCK0_FIELD_DEFINITION_ID);
        idField.value = 0;
        entry.data.add(idField);

        let typeField = new Field(BLOCK0_FIELD_DEFINITION_TYPE, 3);
        typeField.value = 0;
        ...
        let fileNumber = named['fileNumber'].value as number;
        let compressedSize = named['compressedSize'].value as number;
        let flags = named['flags'].value as number;

        let prefix = UNMAPPED_DIR + archive.getArchiveFile(entry).prefix + '/file';
...
export default class MnfReader {
    ...
    async read(path: string): Promise<MnfArchive> {
        ...
        let archive = new MnfArchive(path);
        try {
            let content = await readFile(path);
            let file = new BufferReader(content);
            archive.fileSize = content.byteLength;

            let header = file.readString(FILE_ID.length);
            if (header !== FILE_ID) {
                throw new Error('Invalid .mnf file');
            }

            let version = file.readUInt8();
            if (version !== 3) {
    ...
    getArchive(path: string): MnfArchive {
