import MnfEntry from '../mnf/MnfEntry.js';
import BufferReader, { FieldData, FieldDefinition, FieldType, Field } from '../util/BufferReader.js';
...
export default class ZOSFileTableEntry {
    ...
    readBlock0(segment: number, reader: BufferReader): number {
        let prefix = 'segment' + segment + 'block0';

        let rowCountField = new Field(BLOCK1_FIELD_DEFINITION_ROW_COUNT, -1);
        rowCountField.value = 0;
        this.data.add(rowCountField, prefix);

        let idField = new Field(BLOCK1_FIELD_DEFINITION_ID);
        idField.value = 0;
        this.data.add(idField, prefix);

        let typeField = new Field(BLOCK1_FIELD_DEFINITION_TYPE, 3);
        typeField.value = 0;
    ...
    readFileName(fileNameList: string) {
        let nameOffset = this.data.named['segment1block2nameOffset'].value as number;
        let endIndex = fileNameList.indexOf('\0', nameOffset);
